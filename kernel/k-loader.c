...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    // allocate memory and set up mappings
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
        // Allocate a new physical page
        uintptr_t pa = allocate_free_physical_page();
        if (pa == 0) {
            console_printf(CPOS(22, 0), 0xC000, "program_load_segment(pid %d): can't allocate physical page\n", p->p_pid);
            return -1;
        }
        ...
        if (virtual_memory_map(p->p_pagetable, addr, pa, PAGESIZE, perm) < 0) {
            console_printf(CPOS(22, 0), 0xC000, "program_load_segment(pid %d): can't map address %p\n", p->p_pid, addr);
            return -1;
        }
    }
